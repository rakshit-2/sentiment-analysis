...
interface MeetingLikelihoodChartProps {
  data: DetailedMetricData[];
}

const MeetingLikelihoodChart = ({ data }: MeetingLikelihoodChartProps) => {
  if (data.length === 0) {
    return (
  ...
  const chartData = data.map((day) => ({
    date: day.date,
    likelihood: day.averages.meeting_likelihood,
  }));
...
export default MeetingLikelihoodChart;
